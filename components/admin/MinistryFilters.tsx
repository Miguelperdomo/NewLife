...
import type { MinistryStatus } from "@/lib/admin/types";

export interface MinistryFiltersValue {
  query: string;
  status: "all" | MinistryStatus;
}

const statusOptions: { label: string; value: MinistryFiltersValue["status"] }[] = [
  { label: "Todos", value: "all" },
  { label: "Activos", value: "active" },
  { label: "Archivados", value: "archived" },
];

/** Mismo patrón visual que ContentFilters.tsx: buscador + select de estado. */
export function MinistryFilters({
  value,
  onChange,
}: {
  value: MinistryFiltersValue;
  onChange: (value: MinistryFiltersValue) => void;
}) {
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      <select
        value={value.status}
        onChange={(event) =>
          onChange({ ...value, status: event.target.value as MinistryFiltersValue["status"] })
        }
